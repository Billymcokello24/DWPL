    ...
    <div className="pt-[66px]">
      ...
      <section className="relative overflow-hidden min-h-[46vh] flex items-center">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 w-full">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="section-label mb-5">Solution Portfolio</div>
              <h1 className="font-display font-black text-5xl md:text-6xl text-[var(--text-main)] leading-none mb-6">
                19+ Technology <span className="text-[var(--accent)]">Solution Areas</span>
              </h1>
              <p className="text-lg max-w-xl leading-relaxed text-[var(--text-muted)]">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className="card p-4 text-center cursor-pointer transition-all hover:border-[var(--accent)]"
                >
                  <div className="font-display font-black text-2xl mb-1 text-[var(--accent)]">{c.count}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <div className="relative flex-shrink-0">
              ...
              <input
                ...
                style={{ width: "220px", height: "36px", padding: "0 0.75rem 0 2.25rem" }}
      ...
      <section className="max-w-7xl mx-auto px-6 py-14">
        {filtered.length === 0 ? (
          <div className="text-center py-24 flex flex-col items-center">
            ...
            <p className="text-lg font-display text-[var(--text-main)] mb-2">No solutions found</p>
            <p className="text-sm text-[var(--text-muted)]">Try a different keyword or clear the filter.</p>
            <button onClick={() => { setSearch(""); setActiveCategory("All"); }} className="btn-outline mt-6 text-sm">Clear filters</button>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="p-7">
                    <div className="flex items-start justify-between mb-5">
                      ...
                      <span
                        className="text-xs px-2.5 py-1 rounded-lg font-mono-tech font-semibold"
                    ...
                    <h3 className="font-display font-bold text-xl text-[var(--text-main)] mb-1">{sol.title}</h3>
                    <p className="text-sm mb-4 font-medium text-[var(--accent)]">{sol.tagline}</p>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-5">
                      {sol.features.slice(0, 4).map((f) => (
                        <span key={f} className="text-xs px-2 py-0.5 rounded" style={{ background: "var(--surface)", border: "1px solid var(--card-border)", color: "var(--text-muted)" }}>
                          {f}
                        </span>
                      ))}
                      {sol.features.length > 4 && (
                        <span className="text-xs px-2 py-0.5 rounded" style={{ background: "var(--surface)", border: "1px solid var(--card-border)", color: "var(--text-muted)" }}>
                          +{sol.features.length - 4}
                        </span>
                      )}
                    </div>
      ...
      <section className="band">
        <div className="max-w-7xl mx-auto px-6 py-20 text-center">
          ...
          <p className="text-sm mb-8 max-w-md mx-auto text-[var(--text-muted)]">
            Our technology specialists will assess your environment and recommend the right solution.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={() => navigate("contact")} className="btn-primary">Request a Free Assessment →</button>
            <button onClick={() => navigate("services")} className="btn-outline">View Our Services</button>
          </div>
        </div>
      </section>
    </div>
