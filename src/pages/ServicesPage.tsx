  ...
  return (
    <div className="pt-[66px] min-h-screen pb-20">
      ...
      {servicesData.map((cat, idx) => {
        ...
        return (
          <section key={cat.id} id={cat.id} className={idx % 2 === 1 ? "band py-20" : "py-20"}>
            <div className="max-w-7xl mx-auto px-6">
              <div className="grid lg:grid-cols-2 gap-14 items-center">
                {/* Image side */}
                <div className={`${idx % 2 === 1 ? "order-2" : "order-1"}`}>
                  <div className="relative rounded-2xl overflow-hidden h-96 shadow-2xl border" style={{ borderColor: `color-mix(in srgb, ${cat.color} 30%, transparent)` }}>
                    ...
                    <div className="absolute bottom-6 left-6 right-6 flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs font-mono-tech text-slate-300">{cat.deliverables[0]}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Content side */}
                <div className={`${idx % 2 === 1 ? "order-1" : "order-2"}`}>
                  <div className="section-label mb-3">
                    Service Specialization
                  </div>
                  <h2 className="font-display font-bold text-3xl text-[var(--text-main)] mb-4 leading-tight">
                    {cat.name}
                  </h2>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 mb-8">
                    {cat.capabilities.slice(0, 4).map((cap) => (
                      <div key={cap.title} className="card p-4 border" style={{ borderColor: "var(--card-border)" }}>
      ...
      <section className="band py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-14">
            <span className="section-label mb-4">SLA-Driven Delivery</span>
            <h2 className="font-display font-black text-3xl sm:text-4xl text-[var(--text-main)] mb-4">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {[
              ...
              >
                {pkg.highlight && (
                  <div
                    className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-mono-tech font-bold"
                ...
                <div>
                  <div className="font-display font-black text-2xl text-[var(--text-main)] mb-1">{pkg.name}</div>
                  ...
                  <div className="space-y-4 mb-8 text-sm">
                    <div className="p-3 rounded-xl" style={{ background: "var(--surface)", border: "1px solid var(--card-border)" }}>
                      <div className="text-[0.68rem] uppercase font-mono-tech tracking-wider text-[var(--text-muted)] mb-0.5">Response Time</div>
                      <div className="font-bold text-base text-[var(--accent)]">{pkg.response}</div>
                    </div>

                    <div className="p-3 rounded-xl" style={{ background: "var(--surface)", border: "1px solid var(--card-border)" }}>
                      <div className="text-[0.68rem] uppercase font-mono-tech tracking-wider text-[var(--text-muted)] mb-0.5">Coverage Hours</div>
